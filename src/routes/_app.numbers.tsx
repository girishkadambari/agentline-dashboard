...
function Numbers() {
  ...
  async function loadData() {
    ...
    try {
      ...
      setError(
        caught instanceof VukhoApiError ? formatApiError(caught) : "Could not load numbers.",
      );
    } finally {
      setIsLoading(false);
    }
    ...
    <div>
      ...
      {isLoading ? (
        ...
        <EmptyState
          ...
          description="Import an existing number or provision a new Vukho number for calls and SMS."
          action={
        ...
        {numbers.map((number) => {
          ...
          return (
            <tr
              key={number.id}
              onClick={() =>
                navigate({ to: "/numbers/$numberId", params: { numberId: number.id } })
              }
            ...
            >
              ...
              <td className="px-4 py-3">
                {number.country}
                {number.areaCode ? ` · ${number.areaCode}` : ""}
              </td>
              <td className="px-4 py-3">
              ...
              <td className="px-4 py-3">
                <span className="block truncate">
                  {agent?.name ?? <span className="text-muted-foreground">Unassigned</span>}
                </span>
              </td>
              <td className="px-4 py-3">
              ...
              <td className="px-4 py-3">
                <div className="flex justify-end gap-1.5">
                  ...
                  <button
                    ...
                    disabled={number.status === "released"}
...
function NumberDrawer({
  ...
  async function submit(event: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      if (isCreate || isImport) {
        const capabilities = [sms ? "sms" : null, voice ? "voice" : null].filter(
          Boolean,
        ) as string[];
        const payload = {
  ...
  const description = isCreate
    ? "Provision a real phone number and optionally attach it to an agent. Monthly number charges begin when the number is active."
    : isImport
      ? "Connect a phone number that already exists in your Vukho phone account and attach it to an agent without buying another number."
    ...
    <Sheet open={open} onOpenChange={onOpenChange}>
      <SheetContent side="right" className="w-full overflow-y-auto sm:max-w-xl">
        ...
        <form className="mt-6 space-y-4" onSubmit={submit}>
          ...
          {isImport && <Field label="Phone number" value={phoneNumber} onChange={setPhoneNumber} />}
          {(isCreate || isImport) && (
